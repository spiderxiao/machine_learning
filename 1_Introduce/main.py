...
class NeuralNet(nn.Module):
    ...
    def __init__(self, input_dim):
        ...
        self.net = nn.Sequential(
            nn.Linear(input_dim, 32),#上面选了14个特征，所以这个神经元选32
            nn.BatchNorm1d(32),#批量归一化
            nn.LeakyReLU(),  # 保留一点点负值 max(0.01x, x)
            nn.Dropout(p=0.2),#放在最后
...
save_pred(preds, res_path)         # save prediction file to pred.csv